...
| `c4 list` | 모든 작업자 목록 |
| `c4 close <이름>` | 작업자 종료 |
| `c4 scribe scan` | 모든 세션 스캔하여 컨텍스트 요약 생성 |
| `c4 scribe start\|stop\|status` | 주기적 scribe 스캔 관리 |
...
- **재귀적 C4**: 작업자가 하위 작업자를 생성해서 계층적 작업 위임
- **머지 보호**: `c4 merge`로 테스트/문서 체크 강제. pre-commit hook으로 main 직접 커밋 차단
- **ScreenBuffer**: 가상 터미널이 ANSI 이스케이프 시퀀스 처리 — 깨끗한 텍스트, 스피너 노이즈 없음
- **Scribe**: 세션 컨텍스트 영속화 — JSONL 대화 기록 스캔, 핵심 결정/에러/진행상황 자동 추출, compact 후 PostCompact hook으로 맥락 복원
- **데몬 관리자**: `c4 daemon start/stop/restart/status` PID 추적 및 헬스체크
- **작업자 타임아웃**: 유휴 작업자 감지 (기본 10분) 및 경고 로그
...
**Q: 데몬이 죽으면?**
A: `c4 daemon start`로 재시작하세요. 데몬 죽으면 작업자는 소실됩니다 (세션 복구 기능 개발 예정).
...
전체 로드맵은 [TODO.md](TODO.md)를 참고하세요. 현재 우선순위:

- **Phase 1**: 핵심 기능 (자동 승인, worktree, 데몬 관리자) — 대부분 완료
- **Phase 2**: 운영 안정화 (헬스체크, 타임아웃, 세션 복구)
- **Phase 3**: 고급 기능 (서브에이전트 스웜, hook 기반 아키텍처, 플래너 작업자)
