...
**Task 플래그**: `--branch`, `--after <작업자>`, `--scope <json>`, `--scope-preset`, `--context <작업자>`, `--reuse`, `--template`, `--auto-mode`
...
**오케스트레이션**
- **작업 큐**: 동시 실행 제한 (`maxWorkers`), 의존성 (`--after`), 중복 방지
...
- **Watchdog**: 자율 운영 — 데몬/관리자/scribe 자동 재시작
...
**Q: 데몬이 죽으면?**
A: `c4 daemon start`로 재시작하세요. `watchdog.sh`를 사용하면 무인 운영 가능 — 데몬, 관리자, scribe 자동 재시작합니다.
...
Phase 1/2/3 전체 45개 항목 구현 완료. 자세한 내용은 [TODO.md](TODO.md)와 [CHANGELOG.md](CHANGELOG.md) 참고.
