...
> **상태**: 테스트 및 검증 완료 — 관리자 → 하위 작업자 전체 생명주기(create/task/wait/read/close) 확인됨.
...
## Phase 9-12 신규 기능 (1.6.16 - 1.6.17)

워커 매니저 위에 Fleet, 스케줄러, 감사 로그, 칸반 보드, 워크플로우 엔진,
자연어 입력이 추가됐습니다. 기본은 모두 옵트인이라 단일 머신/단일 사용자
사용에는 영향 없음.

| 영역 | 핵심 |
|------|------|
| **어댑터** (`src/adapters/`) | `claude-code` (기본), `local-llm`, `computer-use` (non-PTY). 워커 spawn + 패턴 검출이 워커별로 라우팅. |
| **Fleet** (9.6 / 9.7 / 9.8) | `config.fleet.peers`로 peer daemon 등록. `c4 fleet peers/list`, `c4 dispatch <task>` (least-load / round-robin / tag-match), `POST /fleet/transfer` rsync. |
| **RBAC + Audit** (10.1 / 10.2) | HMAC bearer 토큰, viewer/manager/admin 3단계, append-only `logs/audit.jsonl`, Web UI 로그인 폼. |
| **Projects / Cost / Departments** (10.3 / 10.5 / 10.6) | worktree → project 매핑, 일별 토큰 비용 집계 + 예산 알림, 부서 멤버/quota 추적. |
| **Scheduler** (10.7) | 5필드 cron parser, `config.schedules` + 런타임 추가/제거 `POST /schedule`. `target='dispatch'`이면 9.7 dispatcher 경유. |
| **PM kanban** (10.8) | 추가 전용 보드 JSONL, 드래그앤드롭 Web UI, TODO.md import. |
| **MCP 허브** (11.1) | `config.mcp.servers`를 워커 `.claude/settings.json`에 profile/option별 inject. |
| **Workflow 엔진** (11.3) | JSON DAG + `dependsOn`/`on_failure`, 빌트인 액션 (task/dispatch/wait/shell/notify/sleep/list/create/close/schedule), `logs/workflow-runs.jsonl` 영속. |
| **NL 인터페이스** (11.4) | heuristic intent → workflow 플랜. `config.nl.llm.enabled` + `ANTHROPIC_API_KEY`로 LLM 폴백. |
| **CI/CD webhooks** (10.4) | `POST /webhook/github` (HMAC), `POST /webhook/gitlab` (token)가 자동으로 review/deploy 워커 spawn. |
| **Web UI** | 대시보드에 Projects, Fleet, Cost, Departments, Board, Scheduler, Audit, Workflow, Context 뷰 + 글로벌 NL 커맨드 바. |
| **OpenAPI 3.1** | `GET /openapi.json` (auth 불필요)로 모든 라우트 자가 기술. |
| **데몬 SPA 서빙** (1.6.17 / #94) | `c4 daemon start`만으로 `web/dist/` 정적 서빙 + SPA fallback + immutable 자산 캐시. `vite preview` 불필요. |
| **워커 CPU/RSS 메트릭** (1.6.17 / #95) | Linux `/proc` 샘플링으로 `/list`에 cpuPct/rssKb/threads, `GET /metrics`에 데몬+워커 합계. Web UI MetricsBar. |
| **멀티 레포 workspaces** (1.6.17 / #98) | `config.workspaces` 매핑 + `c4 task --workspace <name>`. `--repo` 없이 다른 레포 dispatch. |
| **워커 계층 트리** (1.6.17 / 8.2) | `_parent` 필드 + 사이드바 indent + connector + "{n} sub" 뱃지. |
| **부서 월간 $ 예산 + tier** (1.6.17 / 8.3) | `config.departments[*].monthlyBudgetUSD` + `tier`. `quotaCheck`이 워커 수와 누적 비용 모두 검사. |
| **실패 패턴 힌트** (1.6.17 / 8.4) | 13개 큐레이션 패턴(ENOSPC/ESLint/rate-limit/OOM/...) → 행동 가능한 짧은 힌트가 워커 카드에 노란 뱃지로. |
| **Excel 호환 CSV** (1.6.17 / #97) | `/audit/export?format=csv`가 UTF-8 BOM + CRLF로 응답해 한글이 그대로 보임. |
| **PM 보드 ↔ TODO.md 양방향** (1.6.17 / 10.8) | `pm.todoSync`로 카드 이동이 원본 TODO.md 행 status 셀에 자동 반영. |

### CLI 치트시트 (신규 명령)

```bash
c4 dispatch "어제 PR 리뷰" --tags review --strategy least-load
c4 fleet peers
c4 fleet list
c4 batch-action close worker1 worker2 worker3
c4 restart worker1                  # close + respawn (세션 자동 resume)
c4 cancel  worker1                  # Ctrl+C × 2
c4 suspend worker1                  # SIGSTOP (Unix만)
...
c4 nl      "매일 9시에 리포트 보내줘"
c4 workflow run my-workflow.json
...
활성화하면 Web UI는 로그인 폼을 띄우고 발급된 토큰을 localStorage에 저장하며,
모든 API 호출에 `Authorization: Bearer <token>`을 자동으로 붙입니다.

## SDK (프로그래밍 API)

`c4-cli/sdk`는 daemon HTTP API의 얇은 클라이언트로, CLI를 거치지 않고 Node
스크립트에서 워커를 다룰 때 사용합니다.
...
await c4.task('reviewer', '이 PR 리뷰해줘', { branch: 'c4/review' });
...
- 워커 라이프사이클: `create`, `task`, `send`, `key`, `approve`, `suspend`, `resume`, `restart`, `cancel`, `rollback`, `merge`, `close`
- 읽기: `read`, `readNow`, `scrollback`, `wait`, `waitMulti({ mode: 'all' })`, `list`, `history`
- Scribe: `scribeStart` / `scribeStop` / `scribeStatus` / `scribeContext` / `scribeScan`
- Fleet: `fleetPeers`, `fleetList`, `fleetCreate`, `fleetTask`, `fleetClose`, `fleetSend`, `dispatch`, `transfer`
- Workflow: `runWorkflow`, `workflowRuns`, `workflowTemplates`, `saveWorkflowTemplate`, `deleteWorkflowTemplate`
- Auth: `login`, `whoami`, `withToken(token)`
- Audit / Schedule / Board / Cost / Departments / NL: 같은 이름의 메서드
- 라이브 스트림: `events({ onMessage })`로 SSE 핸들 (`{ close }`)
- 헬퍼: `untilIdle(name, { timeoutMs })`, `openapi()`

TypeScript 타입은 `c4-cli/sdk`에 포함되어 있습니다 (`src/sdk.d.ts`).
...
Phase 1-7 전체 139개 이상 항목 구현 완료. Phase 8 (Web UI, 관측성, 토큰 최적화) 예정. 자세한 내용은 [TODO.md](TODO.md)와 [CHANGELOG.md](CHANGELOG.md) 참고.
