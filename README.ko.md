...
![Version](https://img.shields.io/badge/version-1.10.118-green.svg)
...
**다국어 지원**
- **이중 언어 UI (en/ko)**: 웹 대시보드 완전 한국어화. 브라우저 언어 자동 감지 + localStorage에 저장 + 헤더에서 토글 가능
- **i18n 번들**: 1326+ 키 (탭, 모달, drawer, feature 페이지, 툴팁, 에러 fallback, 확인 다이얼로그, 상태 배지)
- **로케일 전환 안전**: 톤 감지 (destructive vs success 스타일링)는 메시지 텍스트 sniff 대신 별도 boolean state 사용 — 번역 후 silent 회귀 없음
- **시각 회귀 가드**: `npm run lint:i18n-visual` — Playwright로 ~42개 surface walk + 렌더된 DOM에서 영어 UI leak 스캔, 출시 전 자동 검증
...
3. 변경사항을 커밋하세요
4. 로컬에서 `npm run check` 실행 — lint + 테스트 + 웹 빌드 + 번들 사이즈 예산을 한 번에 검사
5. 브랜치를 Push하세요
6. Pull Request를 열어주세요

**사용 가능한 검증:**
- `npm run lint` — OpenAPI 스펙, schema-drift, i18n 번들 lockstep
- `npm test` — 단위/통합 테스트 (201개)
- `npm run lint:bundle-size` — first-paint 청크 예산
- `npm run lint:i18n-visual` — Playwright로 ko 로케일 ~42 surface walk (port 3458 데몬 필요)
- `npm run lint:runtime-drift` — 라이브 데몬 GET 라우트 OpenAPI 검증 (port 3456 데몬 필요)
- `npm run check` — 위 lint + 테스트 + 빌드 + 번들 예산 한 번에

[TODO.md](TODO.md)에서 로드맵과 미완료 작업을 확인하세요.

## 로드맵

Phase 1-8 전체 220+ 항목 구현 완료 (v1.10.527). Web UI는 한/영 i18n (en/ko 1329 keys), WCAG 2.1 AA 접근성, ~370KB first-paint critical path, 9개 자동 품질 게이트 (lint/test/bundle/i18n×2/visual/a11y/runtime/snapshot), ErrorBoundary 보호 포함. 자세한 phase 로그는 [TODO.md](TODO.md), 버전 히스토리는 [CHANGELOG.md](CHANGELOG.md) 참고.
