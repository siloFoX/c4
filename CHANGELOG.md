...
- **실패 패턴 자동 감지 + 힌트** (TODO 8.4 / #101) — `src/failure-patterns.js`에 13개 큐레이션 패턴(ENOSPC/EACCES/git conflict/git dirty/ESLint/rate-limit/auth/OOM/port collision/test fail/timeout 등) → 짧고 행동 가능한 "suggested fix" 힌트 매핑. `findHint(errorHistory, recentText)`가 반복 빈도 높은 항목을 우선 반환. `manager._computeFailureHint(worker)`가 list() 응답에 `failureHint: {id, label, hint, sample, count}` 추가하고 (errCount, snapshotIndex)로 메모이제이션. WorkerList에 노란 알림 박스로 label + hint + count 뱃지 표시 (sample은 hover 툴팁). 신규 테스트 10개 (failure-patterns). 자동 복구는 명시적으로 out of scope — 관찰성 + 행동 힌트가 목표.
