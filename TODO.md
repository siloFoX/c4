...
| 1.10 | `c4 init` 개선 | **done** | claude 경로 자동 감지 → config.json 저장, npm link → ~/.local/bin 심링크 → .bashrc alias 3단계 폴백, EPERM 처리 |
...
| 1.15 | effort 자동 설정 안정화 | todo | /model auto-setup이 간헐적 실패. TUI 버전별 타이밍 차이 대응 필요 |
