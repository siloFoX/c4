...
| 7.21 | c4 wait --all intervention 감지 개선 | **todo** | `c4 wait --all`이 intervention worker 때문에 무한 대기. idle worker가 있어도 intervention worker가 블록. 개선: (1) `--all` 기본 동작을 `--interrupt-on-intervention` 으로 변경, (2) idle 완료된 worker는 즉시 반환하고 intervention worker 별도 보고, (3) 관리자가 wait 결과에서 intervention worker를 바로 처리할 수 있게 출력 개선. |
...
| 8.11 | 재현성 있는 fresh install 검증 (누구나 깔아서 동일하게 동작) | **todo** | 8.10 완료 후 진행. 목표: 다른 머신/사용자가 `git clone` → `npm install` → `npm link` → `c4 init` → 데몬 기동 → 웹 UI 접근까지 끊김 없이 재현 가능하게. 검증 범위: (1) 임시 디렉토리에 repo clone 또는 복사 후 깨끗한 환경에서 전체 설치 플로우 시뮬레이션, (2) 각 단계에서 막히는 부분(누락 dep, hardcoded path, 권한 등) 식별해서 코드/문서 수정, (3) package.json scripts 검토 (dev/build/start/test 일관성), (4) README 최상단에 "Install" 섹션으로 단계별 가이드 명시(Windows PowerShell / Linux bash 분기), (5) Node.js 최소 버전 / 외부 도구(git, tmux, etc.) 명시, (6) 선택적으로 `c4 doctor` 또는 `bin/c4-install-check` 스크립트로 설치 상태 자동 검증 (필수 조건 누락 시 명확한 메시지). 검증: 40 서버 외의 머신(DGX 등)에서 fresh clone → c4 init → 웹 UI 접근까지 끊김 없이 완료. |
| 8.12 | (신규) daemon static serve web/dist + build pipeline | **in-progress** | 큐 v2 task 5로 진행 중. daemon이 3456 포트 하나에서 API + built web UI 동시 서빙. npm run build로 web/dist 생성, express.static middleware + SPA fallback. c4 init/daemon start 시 빌드 자동화. 포트포워딩 1개로 웹 접근. 스펙: /root/c4/docs/tasks/web-serve.md |
...
| 8.14 | **긴급** Web UI 세션 관리 + 인증 (injection 차단) | **todo** | 현재 daemon/Web UI에 **인증 없음**. 포트포워딩 또는 LAN 노출 시 누구나 worker 생성/임의 task 전송/승인/머지/git push 가능 → critical RCE/injection 위험. 8.1에서 JWT 언급 있으나 미구현. 수정: (1) daemon HTTP 서버에 인증 미들웨어 (세션 쿠키 또는 Bearer token), (2) POST /login 엔드포인트: username/password → bcrypt 비교 → JWT 발급 (config.json `auth.users` 또는 별도 `users.json` 관리), (3) 모든 /api/* 라우트에 인증 강제 (예외: /health, /login), (4) Web UI 로그인 페이지 + 토큰 localStorage 저장 + axios 인터셉터로 Authorization 헤더, (5) 로그아웃 + 세션 만료 처리, (6) config에 `auth.enabled` flag — 로컬 전용 사용 시 false 가능하되 기본 true, (7) daemon bindHost가 localhost 외일 때 auth.enabled=false면 명확 경고, (8) CSRF 방어 검토 (SameSite cookie 등), (9) **`c4 init` 비대화형 모드 지원: `c4 init --user <name> --password-file <path>` — password-file 읽어서 bcrypt 해시 후 config 저장, 원본 파일 건드리지 않음 (자동화 세팅용)**. 검증: 인증 없이 /api/* 호출 → 401, 유효 토큰으로 호출 → 정상. 우회 시도 (localStorage 변조 등) 차단 확인. 구현 완료 전까지 daemon/vite 외부 바인딩 금지. 재현 근거: 2026-04-17 포트포워딩 세션 관점의 injection 공격 가능성 사용자 지적. |
...
| 9.9 | Manager-Worker validation object (hallucination spiral 방지) | **todo** | 현재 매니저가 워커의 "완료" 텍스트 보고를 거의 그대로 신뢰. 워커가 거짓/부정확 보고 시 매니저가 감지 못 하고 후속 진행 → hallucination spiral. 수정: 워커 완료 시 구조화된 validation object를 강제 반환 — `{test_passed: bool, test_count: int, files_changed: [...], merge_commit_hash: str, lint_clean: bool, 구현_요약: str}`. 매니저가 git log/diff로 교차 검증. 불일치 시 자동 재작업 지시 또는 사용자 알림. Gemini 피드백 (2026-04-17) 근거. |
| 9.10 | Cost/retry guardrails (financial safety) | **todo** | **긴급** 무인 운영 중 에이전트 무한 수정 루프로 하룻밤 수십만원 토큰 폭주 가능. 현재 하드 리밋 없음. Claude CLI는 `--max-budget-usd` 옵션 있음 (근거: `claude --help` 출력). 수정: (1) task 시작 시 `--max-budget-usd <amount>` 자동 부여 (config 기본값 + task별 override), (2) 워커/매니저 `maxRetries` 카운터 (기본 3, 초과 시 safety stop), (3) 초과 시 Slack/email 알림 + idle 대기, (4) Web UI에 실시간 누적 비용 표시, (5) daemon이 worker별 누적 토큰/비용 집계해서 `c4 token-usage --per-task` 지원. Gemini 피드백 근거. |
| 9.11 | Worktree GC 자동화 (좀비 방지) | **todo** | `c4 cleanup` 명령 존재하지만 수동. 장시간 자율 운영 시 비정상 종료된 워커의 worktree/브랜치가 누적 → 디스크/메모리 leak. 수정: (1) daemon 내부에 주기(기본 1h) GC 루프: inactive worktree(daemon session 없음 + N일 이상 touch 안 됨) 감지, (2) 감지된 고아 worktree를 `git worktree remove --force` + 브랜치 삭제 (머지됐거나 main만 새로운 경우), (3) 미머지 변경 있으면 자동 stash 후 보관 (실수 방지), (4) config로 ttl/interval 조정 가능, (5) c4 cleanup --auto flag로 수동 즉시 실행도 유지. Gemini 피드백 근거. |
