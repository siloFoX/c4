...
| 7.26 | manager 복합 명령 halt 방지 | **todo** | 관리자가 `c4 send ... 2>&1 \| tail -1` + `sleep` + `c4 key ...` + `for dir in ...; do git -C $dir log; done` 같은 복합/파이프/루프 명령을 생성하면 `Bash(c4:*)`/`Bash(git:*)` prefix 권한 매처가 전체를 안전 판정 못 해서 permission prompt에서 halt. CLAUDE.md에 "복합 명령 금지" 명시돼 있지만 실제로는 반복 위반. 수정: (1) .claude/agents/manager.md에 "복합/파이프/루프 명령 절대 금지, 병렬 tool call로 분리" 규칙 강화 + 위반 예시 포함, (2) 권한 allow에 자주 쓰는 안전 패턴 추가 검토 (단, 위험 증가 유의), (3) c4 wait 사용 의무화(sleep 대체), (4) 관리자 PostToolUse hook으로 Bash 복합 명령 감지 시 경고/차단. 재현 근거: 2026-04-17 관리자 세션이 riva-fix-a 커밋 확인용 명령을 복합 형태로 생성해 halt. |
