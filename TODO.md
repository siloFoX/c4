...
| 7.25 | c4 init 시 git identity 설정 (자동 halt 방지) | **todo** | 야간 자동 실행 시 `c4 merge`가 git identity 부재로 실패 → 관리자가 `GIT_AUTHOR_NAME=... c4 merge` env 변수 workaround 시도 → `Bash(c4:*)` 권한 패턴이 env 변수 접두사와 매치 안 되어 permission prompt에서 halt. 아침에 진행된 작업 0개. 핵심 수정: c4 init이 `git config user.email`/`user.name` 존재 확인, 없으면 대화형 입력 받아서 `git config --global` 설정. 기존 c4 init 유저 대응: `c4 daemon start` 시 동일 체크, 미설정이면 경고 출력 + 설정 가이드 안내. 보조 방어: (1) c4 merge가 identity 부재 감지 시 "git config user.email/name 설정 필요" 명확 에러 출력 후 종료, workaround 시도 금지, (2) 관리자 에이전트 규칙에 "머지 실패 시 env 변수 workaround 하지 말고 유저에게 알릴 것" 추가. 검증: fresh 환경에서 git config 없는 상태로 c4 init → 정상 완료 + identity 설정 / 기존 환경에서 identity 있으면 기존 유지. 재현 근거: 2026-04-17 관리자 세션 실측. |
| 7.26 | manager 명령 생성 halt 방지 (복합/파이프/루프/markdown) | **todo** | **halt 모드 A** — 복합/파이프/루프 명령: 관리자가 `c4 send ... 2>&1 \| tail -1` + `sleep` + `c4 key ...` + `for dir in ...; do git -C $dir log; done` 같은 복합/파이프/루프 명령을 생성하면 `Bash(c4:*)`/`Bash(git:*)` prefix 권한 매처가 전체를 안전 판정 못 해서 halt. **halt 모드 B** — markdown 헤더: `c4 task git-identity --auto-mode "... ## 목표 ...\\n### 1. src/cli.js ..."` 형태로 작업 프롬프트에 markdown 헤더(`\\n#`, `\\n##`) 포함하면 "Newline followed by # inside a quoted argument can hide arguments from path validation" 보안 경고로 halt. 긴 작업 프롬프트는 대부분 markdown 구조 사용하므로 반복 발생. CLAUDE.md에 "복합 명령 금지" 명시돼 있지만 실제로는 두 모드 모두 반복 위반. 수정: (1) .claude/agents/manager.md에 "복합/파이프/루프 명령 절대 금지 + markdown 헤더 포함 작업 프롬프트는 파일에 먼저 쓰고 경로로 전달" 규칙 강화 + 위반 예시 포함, (2) 권한 allow 패턴 확대 검토 (위험 증가 유의), (3) c4 wait 사용 의무화(sleep 대체), (4) c4 task가 긴 프롬프트 / markdown 포함 프롬프트 자동 감지 → `.c4-task.md` 파일로 전달 (CLAUDE.md에 1000자+ 자동 file 언급 있음 — 실제 동작 검증 필요), (5) 관리자 PostToolUse hook으로 halt 유발 패턴 사전 차단. 재현 근거: 2026-04-17 관리자 세션이 (A) riva-fix-a 커밋 확인용 복합 명령으로 halt, (B) 7.25 git-identity 작업 프롬프트의 `## 목표`/`### 1.` markdown 헤더로 halt. |
| 7.27 | src/cli.js 실행 bit 반복 상실 | **todo** | 머지/stash-pop 후 src/cli.js가 실행권한(100755→100644)을 반복적으로 잃음 → `c4 health`가 `/usr/bin/c4: Permission denied (exit 126)`로 실패 → 매니저가 매 머지마다 `git update-index --chmod=+x src/cli.js` 로 수동 복구. 조사: (1) git diff에서 old mode 100644 / new mode 100755 교차 발생 지점 추적, (2) .gitattributes / core.filemode 설정 확인, (3) 워커 worktree의 파일 권한이 main으로 전파되는 메커니즘, (4) pre-commit hook(.githooks/pre-commit)이 chmod 영향 주는지. 최소 방어: repo에 `chmod +x src/cli.js` 상태를 permanent commit + .gitattributes로 고정. 재현 근거: 2026-04-17 flag-adder/name-adder 머지 흐름에서 2회 관찰. |
| 7.28 | c4 merge가 uncommitted 로컬 변경 guard | **todo** | 머지 대상 main에 uncommitted 변경(TODO.md, package-lock.json 등)이 있으면 매니저가 stash→merge→pop 순서로 수동 처리해야 함. pop 시 conflict 발생하면 추가 수동 resolve 필요. 반복되면 시간/에러 증가, 자동 시스템에서는 halt 유발. 수정: (1) c4 merge가 `git status --porcelain` 체크해서 uncommitted 변경 있으면 "커밋하거나 stash 먼저 하세요" 명확 에러 후 종료, (2) 또는 `--auto-stash` 플래그로 자동 stash→pop 지원하되 conflict 시 명확 안내, (3) 관리자 에이전트 규칙에 "머지 전 working tree clean 확인" 추가. 재현 근거: 2026-04-17 두 차례 머지 중 TODO.md/package-lock.json stash/pop conflict 수동 resolve 필요. |
| 7.29 | package-lock.json 자발적 수정 원인 추적 | **todo** | 세션 시작부터 package-lock.json이 `M` 상태. 의도적 수정 없이도 반복 등장. 머지 때마다 stash 대상이 되고 의미 없는 diff 생성. 조사: (1) `git diff package-lock.json`의 실제 변경 내용이 lockfileVersion/의존성 순서/hash 등 어디인지 확인, (2) `npm install` 없이도 수정되는지 (daemon 기동이 건드리는지), (3) Windows/Linux 간 동기화 부작용 여부, (4) npm 버전 차이 영향. 수정 방향: 원인 제거 또는 package-lock.json을 .gitignore 후보 검토 (단, 의존성 재현성 포기 tradeoff). 재현 근거: 2026-04-17 세션 초기 상태부터 M, 머지마다 방해. |
...
| 8.10 | Web UI 외부 접근 지원 (vite + daemon host 바인딩 + c4 init 통합) | **todo** | 현재 vite.config.ts에 `host` 설정 없어 기본 localhost 바인딩. `http://192.168.10.40:5173/` 같은 외부 IP 접근 시 Connection refused. c4 daemon도 `127.0.0.1:3456` 로컬 바인딩이라 설령 vite가 외부 오픈돼도 `/api/*` 프록시 호출 실패 가능. 수정: (1) vite.config.ts에 `server.host: '0.0.0.0'` 추가 (또는 `server.host: true`), (2) c4 daemon bind host를 config.json으로 설정 가능하게 (기본 localhost 유지, 옵션으로 `0.0.0.0` 또는 LAN IP), (3) **c4 init 통합** — init 과정에서 "Web UI 외부 접근 활성화? (y/N)" 질문, yes면 vite.config.ts의 host 자동 설정 + config.json에 daemon bind host 설정 + 현재 머신 IP 자동 감지해서 접근 URL 안내 출력 (`✓ http://192.168.10.40:5173/ 로 접근 가능`), (4) README/handoff에 외부 접근 시작 가이드, (5) 보안 검토: 0.0.0.0 바인딩 시 JWT 인증(8.1 계획) 의존성, 방화벽 경고 출력. init이 7.25(git identity) + 8.10(외부 접근) 둘 다 묶어서 한 번에 setup 완료하는 흐름. 재현 근거: 2026-04-17 40 서버(192.168.10.40)에서 `curl http://192.168.10.40:5173/` → Connection refused. 포트 미리스닝 + config 부재 둘 다 확인. |
