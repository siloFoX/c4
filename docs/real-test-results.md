...
> Tested: 2026-04-03 21:02 KST
> Node.js: v24.11.1 | Platform: Win11 | node-pty: OK
> Method: Node.js direct module execution (no daemon start/stop, no worker creation)
...
| **allow src/foo.js (glob `src/**/*.js`)** | **FAIL** |
...
**FAIL 원인**: `_matchGlob`에서 `**`가 0개 디렉토리를 매칭하지 못함.
- `src/**/*.js` -> regex `(^|/)src/.*/[^/]*\.js` -> `src/foo.js`에서 `/`가 하나뿐이라 매칭 실패
- `src/utils/foo.js`는 정상 매칭
- 대부분의 glob 라이브러리는 `**`가 0+개 디렉토리를 매칭하므로 edge case
...
1. **ScopeGuard `_matchGlob` zero-depth `**`**: `src/**/*.js` 패턴이 `src/foo.js`를 매칭하지 못함. `**`가 1+개 디렉토리만 매칭하고 0개를 매칭하지 않음. regex 변환 시 `src/.*/` 부분이 반드시 `/`를 요구하기 때문.

## Test Script

`tests/_real_test.js` (160 assertions, ~660 lines)
