  ...
  'POST /meetings/:id/publish': {
    ...
    requestBody: {
      properties: {
        ...
        gitCommit: { type: 'boolean', description: '(Phase 3.4) After writing pages, git init (if needed) + git add + git commit. Best-effort.' },
        gitPush: { type: 'boolean', description: '(Phase 3.4) Also push to origin (requires gitCommit + a configured remote)' },
      },
  ...
  'POST /meetings/:id/run': {
    ...
    requestBody: {
      properties: {
        ...
        autoFinalize: { type: 'boolean', description: 'After run() reaches terminal status, also compute retro deltas + apply them to the registry score record' },
        autoPublish: { type: 'boolean', description: 'After autoFinalize, also publish meeting + retro pages to the wiki (requires autoFinalize=true)' },
        wikiRoot: { type: 'string', description: 'Override default wiki location for autoPublish' },
        alpha: { type: 'number', description: 'Smoothing factor for autoFinalize (default 0.3)' },
        gitCommit: { type: 'boolean', description: '(Phase 3.4) During autoPublish also git init+add+commit the wiki dir' },
        gitPush: { type: 'boolean', description: '(Phase 3.4) After gitCommit, also push to origin' },
      },
      example: { brain: 'mock', autoFinalize: true, autoPublish: false },
    },
    response: {
      properties: {
        ok: { type: 'boolean' },
