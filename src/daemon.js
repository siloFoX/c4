...
// config.openapi.validateResponses; off by default. Implementation
// lives in openapi-validate.checkResponseDriftAndWarn so unit tests
// can exercise it without spawning a daemon subprocess.
function _validateResponseAndWarn(method, route, body, cfgNow) {
  const { checkResponseDriftAndWarn } = require('./openapi-validate');
  const { ROUTE_SCHEMAS } = require('./openapi-gen');
  checkResponseDriftAndWarn(method, route, body, ROUTE_SCHEMAS, cfgNow);
}
...
<body>
  ...
  <script>
    window.ui = SwaggerUIBundle({
      ...
      // (1.11.76) Powers the Auto.tsx dashboard's Live queue table and
      // (1.11.94) the Queue.tsx editor. Reads
      // docs/autonomous-queue-v10.md from the repo root and parses its
      // markdown table into structured rows. The route is intentionally
      // decoupled from the AutoDispatcher instance so the UI can render
      // the queue even when autonomous.mode is off (operators inspect
      // the queue before flipping the flag). Missing file is surfaced
      // as an empty result so the dashboard's empty state can fire
      // instead of an error panel. `raw` carries the verbatim markdown
      // so the editor can show a diff view or fall back to manual
      // editing when the parser gets confused.
      const cfgQueue = manager.getConfig() || {};
      const repoRootQueue = (cfgQueue.worktree && cfgQueue.worktree.projectRoot) || process.cwd();
      const out = queueEditor.handleGetQueueRequest({ repoRoot: repoRootQueue });
      res.writeHead(out.status);
      res.end(JSON.stringify(out.body));
      return;

    } else if (req.method === 'POST' && route === '/autonomous/queue') {
      // (1.11.94) Persist a user-supplied rows[] back to
      // docs/autonomous-queue-v10.md. Validation:
      //   * status in todo|doing|done|partial
      //   * id unique within the table
      //   * detail optional (empty string accepted)
      // On success the markdown is rebuilt with the rows in the new
      // order, written atomically (tmp + rename), and a single audit
      // line is recorded so editor activity is traceable.
      const gate = requireRole(authCheck, rbac.ACTIONS.CONFIG_RELOAD);
      if (denyOr(res, gate)) return;
      const body = await parseBody(req);
      if (_validateOrFail('POST', '/autonomous/queue', body, res, cfg)) return;
      const cfgQueue = manager.getConfig() || {};
      const repoRootQueue = (cfgQueue.worktree && cfgQueue.worktree.projectRoot) || process.cwd();
