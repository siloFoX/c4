...
class PtyManager extends EventEmitter {
  ...
  // (TODO #98) Multi-repo workspace mode.
  //
  ...
  //   }
  //
  // `c4 task --workspace arps "..."` resolves to the matching path and uses it
  // as the projectRoot, so a single daemon can manage workers across multiple
  // checkouts without juggling config.worktree.projectRoot per call.
  listWorkspaces() {
    ...
    for (const [name, p] of Object.entries(map)) {
      ...
      try {
        ...
        // permissions / EPERM — surface as exists:false
      }
