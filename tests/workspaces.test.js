// Multi-repo workspace mode tests (TODO #98).
//
// `config.workspaces` lets a single daemon dispatch tasks against multiple
// repo roots without per-call --repo flags. We cover the two manager APIs
// (listWorkspaces / resolveWorkspace) and ensure unknown / missing-path
// inputs surface helpful errors.
...
describe('listWorkspaces (TODO #98)', () => {
  ...
  it('returns workspaces with resolved paths and existence flags', () => {
    ...
    assert.strictEqual(byName.b.exists, true);
    ...
    assert.strictEqual(byName.gone.isGitRepo, false);
  });

  it('skips entries with empty / non-string paths', () => {
...
describe('resolveWorkspace (TODO #98)', () => {
  ...
  // Cleanup runs after both describe blocks finish.
