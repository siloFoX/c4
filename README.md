...
- **Risk classifier**: 125 patterns (critical/high/medium tiers) with 14 obfuscation defeats (base64, $(), backtick, ${IFS}, \xHH, brace expansion, parameter expansion, multi-stage pipe, quoted-token, etc.) catch hook-time. Per-machine `allowList` / `denyList` / `customRules` overrides. `dryRun` observation mode for tuning. AI second-pass plumbing for external LLM verdicts. `c4 risk patterns [--tier critical|high|medium]` lists the live effective rule set with stable fingerprint. See [`docs/risk-sandbox.md`](docs/risk-sandbox.md).
