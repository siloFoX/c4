...
export function useAuditRotate(): AuditRotateState {
  ...
  const { msg: rotateMsg, failed: rotateFailed, setSuccess, setFailure, reset } =
    useAutoClearMessage();
  ...
  const handleAuditRotate = useCallback(async () => {
    ...
    try {
      ...
      if (res.rotated) {
        setSuccess(tFormat('specialists.rotate.success', {
          archive: res.archive || t('specialists.rotate.fallback'),
        }));
      } else {
        setSuccess(t('specialists.rotate.skipped'));
      }
    } catch (e) {
      setFailure(tFormat('specialists.rotate.failed', {
