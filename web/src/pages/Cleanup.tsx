...
export default function Cleanup() {
  ...
  const commit = useCallback(async () => {
    if (!window.confirm('Remove all listed orphan branches / worktrees / directories? This cannot be undone.')) return;
    setBusy(true);
    setError(null);
    try {
  ...
  return (
    ...
    >
      ...
      {data && total === 0 ? (
        <EmptyPanel message="Nothing to clean up. All workers and worktrees are reconciled." />
      ) : null}
      {data && total > 0 && (
