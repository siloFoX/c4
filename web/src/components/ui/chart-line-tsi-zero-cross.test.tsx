import { describe, expect, it } from 'vitest';
import { render, fireEvent } from '@testing-library/react';
import {
  ChartLineTsiZeroCross,
  DEFAULT_CHART_LINE_TSI_ZERO_CROSS_HEIGHT,
  DEFAULT_CHART_LINE_TSI_ZERO_CROSS_LONG,
  DEFAULT_CHART_LINE_TSI_ZERO_CROSS_PADDING,
  DEFAULT_CHART_LINE_TSI_ZERO_CROSS_PANEL_GAP,
  DEFAULT_CHART_LINE_TSI_ZERO_CROSS_SHORT,
  DEFAULT_CHART_LINE_TSI_ZERO_CROSS_THRESHOLD,
  DEFAULT_CHART_LINE_TSI_ZERO_CROSS_WIDTH,
  applyLineTsiZeroCrossEma,
  classifyLineTsiZeroCrossRegime,
  computeLineTsiZeroCross,
  computeLineTsiZeroCrossLayout,
  describeLineTsiZeroCrossChart,
  detectLineTsiZeroCrossCrosses,
  getLineTsiZeroCrossFinitePoints,
  normalizeLineTsiZeroCrossLength,
  normalizeLineTsiZeroCrossThreshold,
  runLineTsiZeroCross,
  type ChartLineTsiZeroCrossPoint,
} from './chart-line-tsi-zero-cross';

const mk = (closes: number[]): ChartLineTsiZeroCrossPoint[] =>
  closes.map((c, i) => ({ x: i, close: c }));

const SMALL = { long: 5, short: 3 };

describe('getLineTsiZeroCrossFinitePoints', () => {
  it('keeps only finite points', () => {
    const points = [
      { x: 0, close: 10 },
      { x: 1, close: NaN },
      { x: 2, close: 11 },
      { x: Infinity, close: 12 },
      { x: 3, close: -Infinity },
      { x: 4, close: 13 },
    ];
    expect(getLineTsiZeroCrossFinitePoints(points)).toEqual([
      { x: 0, close: 10 },
      { x: 2, close: 11 },
      { x: 4, close: 13 },
    ]);
  });

  it('returns [] for null / undefined / non-array', () => {
    expect(getLineTsiZeroCrossFinitePoints(null)).toEqual([]);
    expect(getLineTsiZeroCrossFinitePoints(undefined)).toEqual([]);
    expect(
      getLineTsiZeroCrossFinitePoints(
        'oops' as unknown as ChartLineTsiZeroCrossPoint[],
      ),
    ).toEqual([]);
  });
});

describe('normalizeLineTsiZeroCrossLength', () => {
  it('keeps finite integers >= 1', () => {
    expect(normalizeLineTsiZeroCrossLength(25, 25)).toBe(25);
    expect(normalizeLineTsiZeroCrossLength(1, 25)).toBe(1);
  });

  it('floors fractional values', () => {
    expect(normalizeLineTsiZeroCrossLength(7.9, 25)).toBe(7);
  });

  it('falls back on invalid input', () => {
    expect(normalizeLineTsiZeroCrossLength(0, 25)).toBe(25);
    expect(normalizeLineTsiZeroCrossLength(-1, 25)).toBe(25);
    expect(normalizeLineTsiZeroCrossLength(NaN, 25)).toBe(25);
  });
});

describe('normalizeLineTsiZeroCrossThreshold', () => {
  it('accepts any finite value', () => {
    expect(normalizeLineTsiZeroCrossThreshold(0, -1)).toBe(0);
    expect(normalizeLineTsiZeroCrossThreshold(20, -1)).toBe(20);
    expect(normalizeLineTsiZeroCrossThreshold(-20, -1)).toBe(-20);
  });

  it('rejects non-finite', () => {
    expect(normalizeLineTsiZeroCrossThreshold(NaN, -1)).toBe(-1);
  });
});

describe('applyLineTsiZeroCrossEma', () => {
  it('CONST after seed -> constant', () => {
    const values: Array<number | null> = [null, 5, 5, 5, 5, 5, 5, 5, 5, 5];
    const out = applyLineTsiZeroCrossEma(values, 5, 1);
    expect(out[5]).toBe(5);
    for (let i = 5; i < 10; i += 1) expect(out[i]).toBe(5);
  });

  it('null seed window -> all null', () => {
    const values: Array<number | null> = [null, 1, null, 3, 4, 5];
    const out = applyLineTsiZeroCrossEma(values, 3, 1);
    expect(out).toEqual([null, null, null, null, null, null]);
  });

  it('length === 1 returns values verbatim from firstValidIdx', () => {
    const out = applyLineTsiZeroCrossEma([null, 1, 2, 3], 1, 1);
    expect(out).toEqual([null, 1, 2, 3]);
  });
});

describe('computeLineTsiZeroCross - CONST K bit-exact anchor', () => {
  it.each([0, 1, 42, 100, 1234])(
    'CONST close K=%d -> tsi = 0 via 0/0 fallback from index long+short onward',
    (K) => {
      const data = mk(new Array(40).fill(K));
      const { tsi } = computeLineTsiZeroCross(data, SMALL);
      // long=5, short=3 -> sm1 valid at i=5, sm2 at i=5+3-1=7.
      for (let i = 0; i < 7; i += 1) {
        expect(tsi[i]).toBeNull();
      }
      for (let i = 7; i < 40; i += 1) {
        expect(tsi[i]).toBe(0);
      }
    },
  );
});

describe('computeLineTsiZeroCross - LINEAR ramps', () => {
  it('LINEAR UP close=i -> tsi = 100', () => {
    const data = mk(Array.from({ length: 40 }, (_, i) => i));
    const { tsi } = computeLineTsiZeroCross(data, SMALL);
    for (let i = 7; i < 40; i += 1) {
      expect(tsi[i]).toBeCloseTo(100, 10);
    }
  });

  it('LINEAR DOWN close=-i -> tsi = -100', () => {
    const data = mk(Array.from({ length: 40 }, (_, i) => -i));
    const { tsi } = computeLineTsiZeroCross(data, SMALL);
    for (let i = 7; i < 40; i += 1) {
      expect(tsi[i]).toBeCloseTo(-100, 10);
    }
  });

  it('tsi[i < 1 + (long-1) + (short-1)] is null', () => {
    const data = mk(Array.from({ length: 40 }, (_, i) => i));
    const { tsi } = computeLineTsiZeroCross(data, SMALL);
    for (let i = 0; i < 7; i += 1) {
      expect(tsi[i]).toBeNull();
    }
  });

  it('custom larger long / short extends warmup', () => {
    const data = mk(new Array(60).fill(10));
    const { tsi } = computeLineTsiZeroCross(data, { long: 10, short: 5 });
    // Valid at i >= 1 + 9 + 4 = 14.
    for (let i = 0; i < 14; i += 1) {
      expect(tsi[i]).toBeNull();
    }
    for (let i = 14; i < 60; i += 1) {
      expect(tsi[i]).toBe(0);
    }
  });
});

describe('classifyLineTsiZeroCrossRegime', () => {
  it('null -> none', () => {
    expect(classifyLineTsiZeroCrossRegime(null, 0)).toBe('none');
  });

  it('tsi at threshold boundary -> bullish', () => {
    expect(classifyLineTsiZeroCrossRegime(0, 0)).toBe('bullish');
    expect(classifyLineTsiZeroCrossRegime(50, 0)).toBe('bullish');
  });

  it('tsi < threshold -> bearish', () => {
    expect(classifyLineTsiZeroCrossRegime(-0.01, 0)).toBe('bearish');
    expect(classifyLineTsiZeroCrossRegime(-100, 0)).toBe('bearish');
  });
});

describe('detectLineTsiZeroCrossCrosses', () => {
  it('fires bullish when tsi crosses up through 0', () => {
    const series = mk([1, 2, 3, 4, 5]);
    const tsi = [-50, -20, -5, 30, 60];
    const crosses = detectLineTsiZeroCrossCrosses(series, tsi, 0);
    expect(crosses).toEqual([{ index: 3, x: 3, kind: 'bullish' }]);
  });

  it('fires bearish when tsi crosses down through 0', () => {
    const series = mk([1, 2, 3, 4, 5]);
    const tsi = [50, 20, 5, -30, -60];
    const crosses = detectLineTsiZeroCrossCrosses(series, tsi, 0);
    expect(crosses).toEqual([{ index: 3, x: 3, kind: 'bearish' }]);
  });

  it('emits bullish then bearish on a sweep', () => {
    const series = mk([1, 2, 3, 4]);
    const tsi = [-50, 50, 25, -50];
    const crosses = detectLineTsiZeroCrossCrosses(series, tsi, 0);
    expect(crosses).toHaveLength(2);
    expect(crosses[0]!.kind).toBe('bullish');
    expect(crosses[1]!.kind).toBe('bearish');
  });

  it('skips when prev or cur is null', () => {
    const series = mk([1, 2, 3]);
    expect(
      detectLineTsiZeroCrossCrosses(series, [null, -50, 50], 0),
    ).toEqual([{ index: 2, x: 2, kind: 'bullish' }]);
    expect(
      detectLineTsiZeroCrossCrosses(series, [-50, null, 50], 0),
    ).toEqual([]);
  });

  it('boundary equality not crossed (strict cur > T)', () => {
    const series = mk([1, 2]);
    expect(detectLineTsiZeroCrossCrosses(series, [-50, 0], 0)).toEqual([]);
  });

  it('no cross when tsi stays on one side', () => {
    const series = mk([1, 2, 3, 4]);
    expect(
      detectLineTsiZeroCrossCrosses(series, [-10, -20, -30, -40], 0),
    ).toEqual([]);
  });

  it('respects custom threshold', () => {
    const series = mk([1, 2, 3]);
    expect(
      detectLineTsiZeroCrossCrosses(series, [10, 15, 25], 20),
    ).toEqual([{ index: 2, x: 2, kind: 'bullish' }]);
  });
});

describe('runLineTsiZeroCross', () => {
  it('CONST K -> 0 crosses, all bullish at boundary after warmup', () => {
    const data = mk(new Array(40).fill(50));
    const run = runLineTsiZeroCross(data, SMALL);
    expect(run.ok).toBe(true);
    expect(run.crosses).toHaveLength(0);
    expect(run.noneCount).toBe(7);
    expect(run.bullishCount).toBe(33);
    expect(run.bearishCount).toBe(0);
    expect(run.long).toBe(5);
    expect(run.short).toBe(3);
  });

  it('LINEAR UP -> all bullish after warmup', () => {
    const data = mk(Array.from({ length: 40 }, (_, i) => i));
    const run = runLineTsiZeroCross(data, SMALL);
    expect(run.crosses).toHaveLength(0);
    expect(run.bullishCount).toBe(33);
    expect(run.bearishCount).toBe(0);
  });

  it('LINEAR DOWN -> all bearish after warmup', () => {
    const data = mk(Array.from({ length: 40 }, (_, i) => -i));
    const run = runLineTsiZeroCross(data, SMALL);
    expect(run.crosses).toHaveLength(0);
    expect(run.bullishCount).toBe(0);
    expect(run.bearishCount).toBe(33);
  });

  it('decline then rise generates bullish cross', () => {
    const closes: number[] = [];
    for (let i = 0; i < 20; i += 1) closes.push(20 - i);
    for (let i = 20; i < 40; i += 1) closes.push(i - 19);
    const run = runLineTsiZeroCross(mk(closes), SMALL);
    expect(run.ok).toBe(true);
    expect(run.crosses.length).toBeGreaterThan(0);
    const kinds = run.crosses.map((c) => c.kind);
    expect(kinds).toContain('bullish');
  });

  it('rise then decline generates bearish cross', () => {
    const closes: number[] = [];
    for (let i = 0; i < 20; i += 1) closes.push(i + 1);
    for (let i = 20; i < 40; i += 1) closes.push(40 - i);
    const run = runLineTsiZeroCross(mk(closes), SMALL);
    expect(run.crosses.length).toBeGreaterThan(0);
    const kinds = run.crosses.map((c) => c.kind);
    expect(kinds).toContain('bearish');
  });

  it('threshold normalization clamps non-finite', () => {
    const data = mk(new Array(40).fill(50));
    const run = runLineTsiZeroCross(data, { ...SMALL, threshold: NaN });
    expect(run.threshold).toBe(0);
  });

  it('respects custom threshold', () => {
    const data = mk(new Array(40).fill(50));
    expect(
      runLineTsiZeroCross(data, { ...SMALL, threshold: 25 }).threshold,
    ).toBe(25);
    expect(
      runLineTsiZeroCross(data, { ...SMALL, threshold: -25 }).threshold,
    ).toBe(-25);
  });

  it('empty / insufficient data -> ok=false', () => {
    expect(runLineTsiZeroCross([], SMALL).ok).toBe(false);
    expect(runLineTsiZeroCross(mk([1, 2, 3]), SMALL).ok).toBe(false);
  });

  it('sorts by x', () => {
    const data: ChartLineTsiZeroCrossPoint[] = [
      { x: 3, close: 30 },
      { x: 1, close: 10 },
      { x: 2, close: 20 },
    ];
    const run = runLineTsiZeroCross(data, { long: 1, short: 1 });
    expect(run.series.map((p) => p.x)).toEqual([1, 2, 3]);
  });

  it('samples carry index / x / close / tsi / regime', () => {
    const data = mk(new Array(40).fill(50));
    const run = runLineTsiZeroCross(data, SMALL);
    expect(run.samples).toHaveLength(40);
    for (let i = 0; i < 7; i += 1) {
      expect(run.samples[i]!.tsi).toBeNull();
      expect(run.samples[i]!.regime).toBe('none');
    }
    for (let i = 7; i < 40; i += 1) {
      expect(run.samples[i]!.tsi).toBe(0);
      expect(run.samples[i]!.regime).toBe('bullish');
    }
  });
});

describe('computeLineTsiZeroCrossLayout', () => {
  it('default layout dimensions match defaults', () => {
    const data = mk(new Array(40).fill(50));
    const layout = computeLineTsiZeroCrossLayout({ data, ...SMALL });
    expect(layout.width).toBe(DEFAULT_CHART_LINE_TSI_ZERO_CROSS_WIDTH);
    expect(layout.height).toBe(DEFAULT_CHART_LINE_TSI_ZERO_CROSS_HEIGHT);
    expect(layout.padding).toBe(DEFAULT_CHART_LINE_TSI_ZERO_CROSS_PADDING);
    expect(layout.panelGap).toBe(DEFAULT_CHART_LINE_TSI_ZERO_CROSS_PANEL_GAP);
  });

  it('fixed -100..100 oscillator range', () => {
    const data = mk(Array.from({ length: 40 }, (_, i) => i));
    const layout = computeLineTsiZeroCrossLayout({ data, ...SMALL });
    expect(layout.oscMin).toBe(-100);
    expect(layout.oscMax).toBe(100);
  });

  it('thresholdY sits at panel midpoint when threshold=0', () => {
    const data = mk(new Array(40).fill(10));
    const layout = computeLineTsiZeroCrossLayout({ data, ...SMALL });
    expect(layout.thresholdY).toBeGreaterThan(layout.oscTop);
    expect(layout.thresholdY).toBeLessThan(layout.oscBottom);
    const expected = (layout.oscTop + layout.oscBottom) / 2;
    expect(layout.thresholdY).toBeCloseTo(expected, 5);
  });

  it('empty data -> ok=false', () => {
    const layout = computeLineTsiZeroCrossLayout({ data: [] });
    expect(layout.ok).toBe(false);
    expect(layout.pricePath).toBe('');
    expect(layout.tsiPath).toBe('');
    expect(layout.crossMarkers).toEqual([]);
  });

  it('path uses M then L commands with 2-decimal precision', () => {
    const data = mk(Array.from({ length: 40 }, (_, i) => i));
    const layout = computeLineTsiZeroCrossLayout({ data, ...SMALL });
    expect(layout.pricePath.startsWith('M ')).toBe(true);
    expect(layout.pricePath).toMatch(/^M [\d.]+ [\d.]+( L [\d.]+ [\d.]+)+$/);
  });

  it('tsi path skips null gaps with new M commands', () => {
    const data = mk(Array.from({ length: 40 }, (_, i) => i));
    const layout = computeLineTsiZeroCrossLayout({ data, ...SMALL });
    expect(layout.tsiPath.startsWith('M ')).toBe(true);
    const mCount = (layout.tsiPath.match(/M /g) ?? []).length;
    expect(mCount).toBe(1);
  });

  it('handles single-value price', () => {
    const data = mk(new Array(40).fill(7));
    const layout = computeLineTsiZeroCrossLayout({ data, ...SMALL });
    expect(layout.priceMin).toBe(6);
    expect(layout.priceMax).toBe(8);
  });

  it('cross markers carry kind / cyOsc / cyPrice / cx', () => {
    const closes: number[] = [];
    for (let i = 0; i < 20; i += 1) closes.push(20 - i);
    for (let i = 20; i < 40; i += 1) closes.push(i - 19);
    const layout = computeLineTsiZeroCrossLayout({
      data: mk(closes),
      ...SMALL,
    });
    expect(layout.crossMarkers.length).toBeGreaterThan(0);
    for (const m of layout.crossMarkers) {
      expect(Number.isFinite(m.cyOsc)).toBe(true);
      expect(Number.isFinite(m.cyPrice)).toBe(true);
      expect(Number.isFinite(m.cx)).toBe(true);
      expect(['bullish', 'bearish']).toContain(m.kind);
    }
  });
});

describe('describeLineTsiZeroCrossChart', () => {
  it('"No data" when empty', () => {
    expect(describeLineTsiZeroCrossChart([])).toBe('No data');
  });

  it('describes bar count + parameters', () => {
    const data = mk(new Array(40).fill(50));
    const desc = describeLineTsiZeroCrossChart(data);
    expect(desc).toContain('TSI Zero Cross chart');
    expect(desc).toContain('40 bars');
    expect(desc).toContain('long 25');
    expect(desc).toContain('short 13');
    expect(desc).toContain('threshold 0');
  });
});

describe('ChartLineTsiZeroCross rendering', () => {
  it('renders region + role=img SVG + sr-only desc', () => {
    const data = mk(new Array(40).fill(10));
    const { container, getByRole } = render(
      <ChartLineTsiZeroCross data={data} {...SMALL} />,
    );
    const region = getByRole('region');
    expect(region.getAttribute('aria-label')).toBe('TSI Zero Cross chart');
    const svg = container.querySelector('svg');
    expect(svg).not.toBeNull();
    expect(svg!.getAttribute('role')).toBe('img');
    const desc = container.querySelector(
      '[data-section="chart-line-tsi-zero-cross-aria-desc"]',
    );
    expect(desc?.textContent).toContain('TSI Zero Cross chart');
  });

  it('renders config badge', () => {
    const data = mk(new Array(40).fill(10));
    const { container } = render(
      <ChartLineTsiZeroCross data={data} {...SMALL} />,
    );
    const badge = container.querySelector(
      '[data-section="chart-line-tsi-zero-cross-badge"]',
    );
    expect(badge?.textContent).toContain('long 5');
    expect(badge?.textContent).toContain('short 3');
    expect(badge?.textContent).toContain('threshold 0');
  });

  it('renders legend toggles for price + tsi', () => {
    const data = mk(new Array(40).fill(10));
    const { container } = render(
      <ChartLineTsiZeroCross data={data} {...SMALL} />,
    );
    const buttons = container.querySelectorAll('[data-series-id]');
    expect(buttons).toHaveLength(2);
    expect(buttons[0].getAttribute('data-series-id')).toBe('price');
    expect(buttons[1].getAttribute('data-series-id')).toBe('tsi');
  });

  it('toggles series visibility via legend click', () => {
    const data = mk(new Array(40).fill(10));
    const { container } = render(
      <ChartLineTsiZeroCross data={data} {...SMALL} />,
    );
    const btn = container.querySelector('[data-series-id="tsi"]');
    expect(btn?.getAttribute('aria-pressed')).toBe('true');
    fireEvent.click(btn!);
    expect(btn?.getAttribute('aria-pressed')).toBe('false');
    fireEvent.click(btn!);
    expect(btn?.getAttribute('aria-pressed')).toBe('true');
  });

  it('respects controlled hiddenSeries', () => {
    const data = mk(new Array(40).fill(10));
    const { container, rerender } = render(
      <ChartLineTsiZeroCross
        data={data}
        {...SMALL}
        hiddenSeries={['tsi']}
      />,
    );
    expect(
      container.querySelector(
        '[data-section="chart-line-tsi-zero-cross-tsi-path"]',
      ),
    ).toBeNull();
    rerender(
      <ChartLineTsiZeroCross data={data} {...SMALL} hiddenSeries={[]} />,
    );
    expect(
      container.querySelector(
        '[data-section="chart-line-tsi-zero-cross-tsi-path"]',
      ),
    ).not.toBeNull();
  });

  it('fires onSeriesToggle on click', () => {
    const data = mk(new Array(40).fill(10));
    const events: Array<{ seriesId: string; hidden: boolean }> = [];
    const { container } = render(
      <ChartLineTsiZeroCross
        data={data}
        {...SMALL}
        onSeriesToggle={(e) => events.push(e)}
      />,
    );
    fireEvent.click(container.querySelector('[data-series-id="tsi"]')!);
    expect(events).toEqual([{ seriesId: 'tsi', hidden: true }]);
  });

  it('keyboard Enter / Space toggles legend', () => {
    const data = mk(new Array(40).fill(10));
    const { container } = render(
      <ChartLineTsiZeroCross data={data} {...SMALL} />,
    );
    const btn = container.querySelector(
      '[data-series-id="tsi"]',
    ) as HTMLButtonElement;
    fireEvent.keyDown(btn, { key: 'Enter' });
    expect(btn.getAttribute('aria-pressed')).toBe('false');
    fireEvent.keyDown(btn, { key: ' ' });
    expect(btn.getAttribute('aria-pressed')).toBe('true');
  });

  it('empty data -> "No data" empty section', () => {
    const { container } = render(<ChartLineTsiZeroCross data={[]} />);
    expect(
      container.querySelector(
        '[data-section="chart-line-tsi-zero-cross-empty"]',
      ),
    ).not.toBeNull();
  });

  it('animate=false omits motion-safe class', () => {
    const data = mk(new Array(40).fill(10));
    const { container } = render(
      <ChartLineTsiZeroCross data={data} {...SMALL} animate={false} />,
    );
    const svg = container.querySelector('svg');
    expect(svg?.getAttribute('class')).toBeNull();
  });

  it('animate=true (default) applies motion-safe fade-in class', () => {
    const data = mk(new Array(40).fill(10));
    const { container } = render(
      <ChartLineTsiZeroCross data={data} {...SMALL} />,
    );
    const svg = container.querySelector('svg');
    expect(svg?.getAttribute('class')).toBe('motion-safe:animate-fade-in');
  });

  it('hover sets tooltip target', () => {
    const data = mk(new Array(40).fill(10));
    const { container } = render(
      <ChartLineTsiZeroCross data={data} {...SMALL} />,
    );
    const hovers = container.querySelectorAll(
      '[data-section="chart-line-tsi-zero-cross-hover"]',
    );
    expect(hovers.length).toBeGreaterThan(0);
    fireEvent.mouseEnter(hovers[0]!);
    const tooltip = container.querySelector(
      '[data-section="chart-line-tsi-zero-cross-tooltip"]',
    );
    expect(tooltip).not.toBeNull();
    fireEvent.mouseLeave(hovers[0]!);
  });

  it('renders threshold band by default', () => {
    const data = mk(new Array(40).fill(10));
    const { container } = render(
      <ChartLineTsiZeroCross data={data} {...SMALL} />,
    );
    expect(
      container.querySelector(
        '[data-section="chart-line-tsi-zero-cross-band-threshold"]',
      ),
    ).not.toBeNull();
  });

  it('showBands=false hides band group', () => {
    const data = mk(new Array(40).fill(10));
    const { container } = render(
      <ChartLineTsiZeroCross data={data} {...SMALL} showBands={false} />,
    );
    expect(
      container.querySelector(
        '[data-section="chart-line-tsi-zero-cross-bands"]',
      ),
    ).toBeNull();
  });

  it('showAxis=false / showGrid=false / showLegend=false', () => {
    const data = mk(new Array(40).fill(10));
    const { container } = render(
      <ChartLineTsiZeroCross
        data={data}
        {...SMALL}
        showAxis={false}
        showGrid={false}
        showLegend={false}
        showConfigBadge={false}
      />,
    );
    expect(
      container.querySelector(
        '[data-section="chart-line-tsi-zero-cross-axes"]',
      ),
    ).toBeNull();
    expect(
      container.querySelector(
        '[data-section="chart-line-tsi-zero-cross-grid"]',
      ),
    ).toBeNull();
    expect(
      container.querySelector(
        '[data-section="chart-line-tsi-zero-cross-legend"]',
      ),
    ).toBeNull();
    expect(
      container.querySelector(
        '[data-section="chart-line-tsi-zero-cross-badge"]',
      ),
    ).toBeNull();
  });

  it('renders cross markers and overlays after decline then rise pattern', () => {
    const closes: number[] = [];
    for (let i = 0; i < 20; i += 1) closes.push(20 - i);
    for (let i = 20; i < 40; i += 1) closes.push(i - 19);
    const { container } = render(
      <ChartLineTsiZeroCross data={mk(closes)} {...SMALL} />,
    );
    const crosses = container.querySelectorAll(
      '[data-section^="chart-line-tsi-zero-cross-cross-"]',
    );
    expect(crosses.length).toBeGreaterThan(0);
  });

  it('showCrosses=false hides cross markers', () => {
    const closes: number[] = [];
    for (let i = 0; i < 20; i += 1) closes.push(20 - i);
    for (let i = 20; i < 40; i += 1) closes.push(i - 19);
    const { container } = render(
      <ChartLineTsiZeroCross
        data={mk(closes)}
        {...SMALL}
        showCrosses={false}
      />,
    );
    expect(
      container.querySelector(
        '[data-section="chart-line-tsi-zero-cross-crosses"]',
      ),
    ).toBeNull();
  });

  it('showOverlayCrosses=false hides overlay arrows', () => {
    const closes: number[] = [];
    for (let i = 0; i < 20; i += 1) closes.push(20 - i);
    for (let i = 20; i < 40; i += 1) closes.push(i - 19);
    const { container } = render(
      <ChartLineTsiZeroCross
        data={mk(closes)}
        {...SMALL}
        showOverlayCrosses={false}
      />,
    );
    expect(
      container.querySelector(
        '[data-section="chart-line-tsi-zero-cross-overlay-crosses"]',
      ),
    ).toBeNull();
  });

  it('data-* attrs reflect run counters', () => {
    const data = mk(new Array(40).fill(10));
    const { container } = render(
      <ChartLineTsiZeroCross data={data} {...SMALL} />,
    );
    const region = container.querySelector(
      '[data-section="chart-line-tsi-zero-cross"]',
    );
    expect(region?.getAttribute('data-long')).toBe('5');
    expect(region?.getAttribute('data-short')).toBe('3');
    expect(region?.getAttribute('data-threshold')).toBe('0');
    expect(region?.getAttribute('data-cross-count')).toBe('0');
    expect(region?.getAttribute('data-bullish-count')).toBe('33');
  });

  it('defaults: long=25, short=13, threshold=0', () => {
    expect(DEFAULT_CHART_LINE_TSI_ZERO_CROSS_LONG).toBe(25);
    expect(DEFAULT_CHART_LINE_TSI_ZERO_CROSS_SHORT).toBe(13);
    expect(DEFAULT_CHART_LINE_TSI_ZERO_CROSS_THRESHOLD).toBe(0);
  });

  it('forwardRef returns the wrapping div', () => {
    const data = mk(new Array(40).fill(10));
    const ref = { current: null as HTMLDivElement | null };
    render(<ChartLineTsiZeroCross data={data} {...SMALL} ref={ref} />);
    expect(ref.current).toBeInstanceOf(HTMLDivElement);
    expect(ref.current?.getAttribute('data-section')).toBe(
      'chart-line-tsi-zero-cross',
    );
  });

  it('layout is deterministic across calls for default CONST 40 bars', () => {
    const data = mk(new Array(40).fill(10));
    const a = computeLineTsiZeroCrossLayout({ data, ...SMALL });
    const b = computeLineTsiZeroCrossLayout({ data, ...SMALL });
    expect(a.pricePath).toBe(b.pricePath);
    expect(a.tsiPath).toBe(b.tsiPath);
    expect(a.thresholdY).toBe(b.thresholdY);
    expect(a.run.crosses).toEqual(b.run.crosses);
  });

  it('layout is deterministic across calls for decline-then-rise pattern', () => {
    const closes: number[] = [];
    for (let i = 0; i < 20; i += 1) closes.push(20 - i);
    for (let i = 20; i < 40; i += 1) closes.push(i - 19);
    const data = mk(closes);
    const a = computeLineTsiZeroCrossLayout({ data, ...SMALL });
    const b = computeLineTsiZeroCrossLayout({ data, ...SMALL });
    expect(a.pricePath).toBe(b.pricePath);
    expect(a.tsiPath).toBe(b.tsiPath);
    expect(a.run.tsiValues).toEqual(b.run.tsiValues);
    expect(a.run.crosses).toEqual(b.run.crosses);
  });
});
