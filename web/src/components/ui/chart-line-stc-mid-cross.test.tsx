...
import {
  ChartLineStcMidCross,
  DEFAULT_CHART_LINE_STC_MID_CROSS_CYCLE,
  DEFAULT_CHART_LINE_STC_MID_CROSS_FAST,
  DEFAULT_CHART_LINE_STC_MID_CROSS_HEIGHT,
  DEFAULT_CHART_LINE_STC_MID_CROSS_PADDING,
  DEFAULT_CHART_LINE_STC_MID_CROSS_PANEL_GAP,
  DEFAULT_CHART_LINE_STC_MID_CROSS_SLOW,
  DEFAULT_CHART_LINE_STC_MID_CROSS_THRESHOLD,
  DEFAULT_CHART_LINE_STC_MID_CROSS_WIDTH,
  applyLineStcMidCrossEma,
  classifyLineStcMidCrossRegime,
  computeLineStcMidCross,
  computeLineStcMidCrossLayout,
  describeLineStcMidCrossChart,
  detectLineStcMidCrossCrosses,
  getLineStcMidCrossFinitePoints,
  normalizeLineStcMidCrossLength,
  normalizeLineStcMidCrossThreshold,
  runLineStcMidCross,
  type ChartLineStcMidCrossPoint,
} from './chart-line-stc-mid-cross';

const mk = (closes: number[]): ChartLineStcMidCrossPoint[] =>
  closes.map((c, i) => ({ x: i, close: c }));

const SMALL = { cycle: 3, fast: 5, slow: 10 };

describe('getLineStcMidCrossFinitePoints', () => {
  it('keeps only finite points', () => {
    ...
    expect(getLineStcMidCrossFinitePoints(points)).toEqual([
      { x: 0, close: 10 },
      { x: 2, close: 11 },
      { x: 4, close: 13 },
    ]);
  ...
  it('returns [] for null / undefined / non-array', () => {
    expect(getLineStcMidCrossFinitePoints(null)).toEqual([]);
    expect(getLineStcMidCrossFinitePoints(undefined)).toEqual([]);
    expect(
      getLineStcMidCrossFinitePoints(
        'oops' as unknown as ChartLineStcMidCrossPoint[],
      ),
    ).toEqual([]);
  });
});

describe('normalizeLineStcMidCrossLength', () => {
  it('keeps finite integers >= 1', () => {
    expect(normalizeLineStcMidCrossLength(10, 10)).toBe(10);
    expect(normalizeLineStcMidCrossLength(1, 10)).toBe(1);
  });

  it('floors fractional values', () => {
    expect(normalizeLineStcMidCrossLength(7.9, 10)).toBe(7);
  });

  it('falls back on invalid input', () => {
    expect(normalizeLineStcMidCrossLength(0, 10)).toBe(10);
    expect(normalizeLineStcMidCrossLength(-1, 10)).toBe(10);
    expect(normalizeLineStcMidCrossLength(NaN, 10)).toBe(10);
  });
});

describe('normalizeLineStcMidCrossThreshold', () => {
  it('accepts any finite value', () => {
    expect(normalizeLineStcMidCrossThreshold(50, 50)).toBe(50);
    expect(normalizeLineStcMidCrossThreshold(70, 50)).toBe(70);
    expect(normalizeLineStcMidCrossThreshold(0, 50)).toBe(0);
  });

  it('rejects non-finite', () => {
    expect(normalizeLineStcMidCrossThreshold(NaN, 50)).toBe(50);
  });
});

describe('applyLineStcMidCrossEma', () => {
  it('CONST values -> EMA = value after SMA seed', () => {
    const out = applyLineStcMidCrossEma(new Array(20).fill(5), 10);
    for (let i = 0; i < 9; i += 1) expect(out[i]).toBeNull();
    for (let i = 9; i < 20; i += 1) expect(out[i]).toBe(5);
  });

  it('length === 1 returns verbatim', () => {
    expect(applyLineStcMidCrossEma([1, 2, 3], 1)).toEqual([1, 2, 3]);
  });

  it('LINEAR ramp converges to constant lag', () => {
    const values = Array.from({ length: 30 }, (_, i) => i);
    const out = applyLineStcMidCrossEma(values, 5);
    expect(out[4]).toBe(2);
...
describe('computeLineStcMidCross - CONST K bit-exact anchor', () => {
  it.each([0, 1, 42, 100, 1234])(
    'CONST close K=%d -> stc = 50 from index 13 onward',
    (K) => {
      const data = mk(new Array(40).fill(K));
      const { stc } = computeLineStcMidCross(data, SMALL);
...
describe('computeLineStcMidCross - LINEAR ramps', () => {
  ...
  it('MACD stays constant for LINEAR UP after EMA seed', () => {
    ...
    const { macd } = computeLineStcMidCross(data, SMALL);
    const first = macd[9];
  ...
  it('custom larger cycle / fast / slow extends warmup', () => {
    ...
    const { stc } = computeLineStcMidCross(data, {
      cycle: 5,
      fast: 8,
      slow: 20,
    });
  ...
  it('all internal channels exposed (macd, k1, d1, k2, stc)', () => {
    const data = mk(new Array(40).fill(50));
    const channels = computeLineStcMidCross(data, SMALL);
...
describe('classifyLineStcMidCrossRegime', () => {
  it('null -> none', () => {
    expect(classifyLineStcMidCrossRegime(null, 50)).toBe('none');
  });

  it('stc at threshold boundary -> bullish', () => {
    expect(classifyLineStcMidCrossRegime(50, 50)).toBe('bullish');
    expect(classifyLineStcMidCrossRegime(75, 50)).toBe('bullish');
  });

  it('stc < threshold -> bearish', () => {
    expect(classifyLineStcMidCrossRegime(49.99, 50)).toBe('bearish');
    expect(classifyLineStcMidCrossRegime(0, 50)).toBe('bearish');
  });
});

describe('detectLineStcMidCrossCrosses', () => {
  it('fires bullish when stc crosses up through 50', () => {
    const series = mk([1, 2, 3, 4, 5]);
    const stc = [40, 45, 49, 60, 65];
    const crosses = detectLineStcMidCrossCrosses(series, stc, 50);
  ...
  it('skips when prev or cur is null', () => {
    ...
    expect(
      detectLineStcMidCrossCrosses(series, [null, 40, 60], 50),
    ).toEqual([{ index: 2, x: 2, kind: 'bullish' }]);
    expect(
      detectLineStcMidCrossCrosses(series, [40, null, 60], 50),
    ).toEqual([]);
  });

  it('boundary equality not crossed (strict cur > T)', () => {
    const series = mk([1, 2]);
    expect(detectLineStcMidCrossCrosses(series, [40, 50], 50)).toEqual([]);
  });

  it('no cross when stc stays on one side', () => {
    const series = mk([1, 2, 3, 4]);
    expect(
      detectLineStcMidCrossCrosses(series, [10, 20, 30, 40], 50),
    ).toEqual([]);
  });
  ...
  it('respects custom threshold', () => {
    const series = mk([1, 2, 3]);
    expect(
      detectLineStcMidCrossCrosses(series, [60, 65, 75], 70),
    ).toEqual([{ index: 2, x: 2, kind: 'bullish' }]);
  });
});

describe('runLineStcMidCross', () => {
  it('CONST K -> 0 crosses, all bullish at boundary after warmup', () => {
    const data = mk(new Array(40).fill(50));
    const run = runLineStcMidCross(data, SMALL);
  ...
  it('decline then rise pattern keeps stc populated', () => {
    const closes: number[] = [];
    for (let i = 0; i < 30; i += 1) closes.push(30 - i);
    for (let i = 30; i < 60; i += 1) closes.push(i - 29);
    const run = runLineStcMidCross(mk(closes), SMALL);
    expect(run.ok).toBe(true);
    const validStc = run.stcValues.filter((v): v is number => v != null);
    expect(validStc.length).toBeGreaterThan(0);
  });

  it('threshold normalization clamps non-finite', () => {
    const data = mk(new Array(40).fill(50));
    const run = runLineStcMidCross(data, { ...SMALL, threshold: NaN });
  ...
  it('respects custom threshold', () => {
    const data = mk(new Array(40).fill(50));
    expect(
      runLineStcMidCross(data, { ...SMALL, threshold: 70 }).threshold,
    ).toBe(70);
    expect(
      runLineStcMidCross(data, { ...SMALL, threshold: 30 }).threshold,
    ).toBe(30);
  });

  it('empty / insufficient data -> ok=false', () => {
    expect(runLineStcMidCross([], SMALL).ok).toBe(false);
    expect(runLineStcMidCross(mk([1, 2, 3]), SMALL).ok).toBe(false);
  });

  it('sorts by x', () => {
    const data: ChartLineStcMidCrossPoint[] = [
      { x: 3, close: 30 },
      { x: 1, close: 10 },
      { x: 2, close: 20 },
    ];
    const run = runLineStcMidCross(data, { cycle: 1, fast: 1, slow: 1 });
...
describe('computeLineStcMidCrossLayout', () => {
  it('default layout dimensions match defaults', () => {
    const data = mk(new Array(40).fill(50));
    const layout = computeLineStcMidCrossLayout({ data, ...SMALL });
    expect(layout.width).toBe(DEFAULT_CHART_LINE_STC_MID_CROSS_WIDTH);
    expect(layout.height).toBe(DEFAULT_CHART_LINE_STC_MID_CROSS_HEIGHT);
    expect(layout.padding).toBe(DEFAULT_CHART_LINE_STC_MID_CROSS_PADDING);
    expect(layout.panelGap).toBe(DEFAULT_CHART_LINE_STC_MID_CROSS_PANEL_GAP);
  });
  ...
  it('empty data -> ok=false', () => {
    const layout = computeLineStcMidCrossLayout({ data: [] });
...
describe('describeLineStcMidCrossChart', () => {
  it('"No data" when empty', () => {
    expect(describeLineStcMidCrossChart([])).toBe('No data');
  });

  it('describes bar count + parameters + bias coloring claim', () => {
    const data = mk(new Array(40).fill(50));
    const desc = describeLineStcMidCrossChart(data);
    expect(desc).toContain('STC Mid Cross chart');
...
describe('ChartLineStcMidCross rendering', () => {
  it('renders region + role=img SVG + sr-only desc', () => {
    const data = mk(new Array(40).fill(10));
    const { container, getByRole } = render(
      <ChartLineStcMidCross data={data} {...SMALL} />,
    );
    const region = getByRole('region');
    expect(region.getAttribute('aria-label')).toBe('STC Mid Cross chart');
    ...
    expect(desc?.textContent).toContain('STC Mid Cross chart');
  });
  ...
  it('respects controlled hiddenSeries', () => {
    ...
    rerender(
      <ChartLineStcMidCross data={data} {...SMALL} hiddenSeries={[]} />,
    );
    expect(
      container.querySelector(
        '[data-section="chart-line-stc-mid-cross-stc-path"]',
  ...
  it('empty data -> "No data" empty section', () => {
    const { container } = render(<ChartLineStcMidCross data={[]} />);
    expect(
      container.querySelector(
  ...
  it('animate=false omits motion-safe class', () => {
    const data = mk(new Array(40).fill(10));
    const { container } = render(
      <ChartLineStcMidCross data={data} {...SMALL} animate={false} />,
    );
    const svg = container.querySelector('svg');
  ...
  it('showBands=false hides band group', () => {
    const data = mk(new Array(40).fill(10));
    const { container } = render(
      <ChartLineStcMidCross data={data} {...SMALL} showBands={false} />,
    );
    expect(
      container.querySelector(
        '[data-section="chart-line-stc-mid-cross-bands"]',
      ),
    ).toBeNull();
  });
  ...
  it('showAxis=false / showGrid=false / showLegend=false', () => {
    ...
    expect(
      container.querySelector(
        '[data-section="chart-line-stc-mid-cross-axes"]',
      ),
    ).toBeNull();
    expect(
      container.querySelector(
        '[data-section="chart-line-stc-mid-cross-grid"]',
      ),
    ).toBeNull();
    expect(
      container.querySelector(
  ...
  it('showCrosses=false hides cross markers group', () => {
    const data = mk(new Array(40).fill(10));
    const { container } = render(
      <ChartLineStcMidCross data={data} {...SMALL} showCrosses={false} />,
    );
    expect(
      container.querySelector(
  ...
  it('defaults: cycle=10, fast=23, slow=50, threshold=50', () => {
    expect(DEFAULT_CHART_LINE_STC_MID_CROSS_CYCLE).toBe(10);
    expect(DEFAULT_CHART_LINE_STC_MID_CROSS_FAST).toBe(23);
    expect(DEFAULT_CHART_LINE_STC_MID_CROSS_SLOW).toBe(50);
    expect(DEFAULT_CHART_LINE_STC_MID_CROSS_THRESHOLD).toBe(50);
  });

  it('forwardRef returns the wrapping div', () => {
    const data = mk(new Array(40).fill(10));
    const ref = { current: null as HTMLDivElement | null };
    render(<ChartLineStcMidCross data={data} {...SMALL} ref={ref} />);
    expect(ref.current).toBeInstanceOf(HTMLDivElement);
    expect(ref.current?.getAttribute('data-section')).toBe(
      'chart-line-stc-mid-cross',
    );
  });

  it('layout is deterministic across calls for default CONST 40 bars', () => {
    const data = mk(new Array(40).fill(10));
    const a = computeLineStcMidCrossLayout({ data, ...SMALL });
    const b = computeLineStcMidCrossLayout({ data, ...SMALL });
