...
import {
  ChartLineUoMidCross,
  DEFAULT_CHART_LINE_UO_MID_CROSS_HEIGHT,
  DEFAULT_CHART_LINE_UO_MID_CROSS_LONG,
  DEFAULT_CHART_LINE_UO_MID_CROSS_MID,
  DEFAULT_CHART_LINE_UO_MID_CROSS_PADDING,
  DEFAULT_CHART_LINE_UO_MID_CROSS_PANEL_GAP,
  DEFAULT_CHART_LINE_UO_MID_CROSS_SHORT,
  DEFAULT_CHART_LINE_UO_MID_CROSS_THRESHOLD,
  DEFAULT_CHART_LINE_UO_MID_CROSS_WIDTH,
  applyLineUoMidCrossPressure,
  classifyLineUoMidCrossRegime,
  computeLineUoMidCross,
  computeLineUoMidCrossLayout,
  describeLineUoMidCrossChart,
  detectLineUoMidCrossCrosses,
  getLineUoMidCrossFinitePoints,
  normalizeLineUoMidCrossLength,
  normalizeLineUoMidCrossThreshold,
  runLineUoMidCross,
  type ChartLineUoMidCrossPoint,
} from './chart-line-uo-mid-cross';

const mk = (closes: number[]): ChartLineUoMidCrossPoint[] =>
  closes.map((c, i) => ({ x: i, close: c }));

describe('getLineUoMidCrossFinitePoints', () => {
  it('keeps only finite points', () => {
    ...
    expect(getLineUoMidCrossFinitePoints(points)).toEqual([
      { x: 0, close: 10 },
      { x: 2, close: 11 },
      { x: 4, close: 13 },
    ]);
  ...
  it('returns [] for null / undefined / non-array', () => {
    expect(getLineUoMidCrossFinitePoints(null)).toEqual([]);
    expect(getLineUoMidCrossFinitePoints(undefined)).toEqual([]);
    expect(
      getLineUoMidCrossFinitePoints(
        'oops' as unknown as ChartLineUoMidCrossPoint[],
      ),
    ).toEqual([]);
  });
});

describe('normalizeLineUoMidCrossLength', () => {
  it('keeps finite integers >= 1', () => {
    expect(normalizeLineUoMidCrossLength(7, 14)).toBe(7);
    expect(normalizeLineUoMidCrossLength(1, 14)).toBe(1);
  });

  it('floors fractional values', () => {
    expect(normalizeLineUoMidCrossLength(7.9, 14)).toBe(7);
  });

  it('falls back on invalid input', () => {
    expect(normalizeLineUoMidCrossLength(0, 14)).toBe(14);
    expect(normalizeLineUoMidCrossLength(NaN, 14)).toBe(14);
  });
});

describe('normalizeLineUoMidCrossThreshold', () => {
  it('accepts any finite value', () => {
    expect(normalizeLineUoMidCrossThreshold(50, 50)).toBe(50);
    expect(normalizeLineUoMidCrossThreshold(70, 50)).toBe(70);
  });

  it('rejects non-finite', () => {
    expect(normalizeLineUoMidCrossThreshold(NaN, 50)).toBe(50);
  });
});

describe('applyLineUoMidCrossPressure', () => {
  it('CONST closes -> bp = tr = 0 from i=1', () => {
    const { bp, tr } = applyLineUoMidCrossPressure(new Array(10).fill(50));
...
describe('computeLineUoMidCross - LINEAR ramps', () => {
...
describe('classifyLineUoMidCrossRegime', () => {
  it('null -> none', () => {
    expect(classifyLineUoMidCrossRegime(null, 50)).toBe('none');
  });

  it('uo at threshold boundary -> bullish', () => {
    expect(classifyLineUoMidCrossRegime(50, 50)).toBe('bullish');
    expect(classifyLineUoMidCrossRegime(75, 50)).toBe('bullish');
  });

  it('uo < threshold -> bearish', () => {
    expect(classifyLineUoMidCrossRegime(49.99, 50)).toBe('bearish');
    expect(classifyLineUoMidCrossRegime(0, 50)).toBe('bearish');
  });
});

describe('detectLineUoMidCrossCrosses', () => {
  it('fires bullish when uo crosses up through 50', () => {
    const series = mk([1, 2, 3, 4, 5]);
    const uo = [40, 45, 49, 60, 65];
    const crosses = detectLineUoMidCrossCrosses(series, uo, 50);
  ...
  it('skips when prev or cur is null', () => {
    ...
    expect(
      detectLineUoMidCrossCrosses(series, [null, 40, 60], 50),
    ).toEqual([{ index: 2, x: 2, kind: 'bullish' }]);
    expect(
      detectLineUoMidCrossCrosses(series, [40, null, 60], 50),
    ).toEqual([]);
  });

  it('boundary equality not crossed (strict cur > T)', () => {
    const series = mk([1, 2]);
    expect(detectLineUoMidCrossCrosses(series, [40, 50], 50)).toEqual([]);
  });

  it('no cross when uo stays on one side', () => {
    const series = mk([1, 2, 3, 4]);
    expect(
      detectLineUoMidCrossCrosses(series, [10, 20, 30, 40], 50),
    ).toEqual([]);
  });
  ...
  it('respects custom threshold', () => {
    const series = mk([1, 2, 3]);
    expect(
      detectLineUoMidCrossCrosses(series, [60, 65, 75], 70),
    ).toEqual([{ index: 2, x: 2, kind: 'bullish' }]);
  });
});

describe('runLineUoMidCross', () => {
  it('CONST K -> 0 crosses, all bullish at boundary after warmup', () => {
    const data = mk(new Array(40).fill(50));
    const run = runLineUoMidCross(data);
  ...
  it('decline then rise generates bullish cross', () => {
    ...
    const run = runLineUoMidCross(mk(closes));
    expect(run.ok).toBe(true);
  ...
  it('threshold normalization clamps non-finite', () => {
    ...
    const run = runLineUoMidCross(data, { threshold: NaN });
    expect(run.threshold).toBe(50);
  });

  it('respects custom threshold', () => {
    const data = mk(new Array(40).fill(10));
    expect(runLineUoMidCross(data, { threshold: 70 }).threshold).toBe(70);
    expect(runLineUoMidCross(data, { threshold: 30 }).threshold).toBe(30);
  });

  it('empty / insufficient data -> ok=false', () => {
    expect(runLineUoMidCross([]).ok).toBe(false);
    expect(runLineUoMidCross(mk([1, 2, 3])).ok).toBe(false);
  });

  it('sorts by x', () => {
    const data: ChartLineUoMidCrossPoint[] = [
      { x: 3, close: 30 },
      { x: 1, close: 10 },
      { x: 2, close: 20 },
    ];
...
describe('computeLineUoMidCrossLayout', () => {
  it('default layout dimensions match defaults', () => {
    const data = mk(new Array(40).fill(50));
    const layout = computeLineUoMidCrossLayout({ data });
    expect(layout.width).toBe(DEFAULT_CHART_LINE_UO_MID_CROSS_WIDTH);
    expect(layout.height).toBe(DEFAULT_CHART_LINE_UO_MID_CROSS_HEIGHT);
    expect(layout.padding).toBe(DEFAULT_CHART_LINE_UO_MID_CROSS_PADDING);
    expect(layout.panelGap).toBe(DEFAULT_CHART_LINE_UO_MID_CROSS_PANEL_GAP);
  });
  ...
  it('cross markers carry kind / cyOsc / cyPrice / cx', () => {
    ...
    const layout = computeLineUoMidCrossLayout({ data: mk(closes) });
...
describe('describeLineUoMidCrossChart', () => {
  it('"No data" when empty', () => {
    expect(describeLineUoMidCrossChart([])).toBe('No data');
  });

  it('describes bar count + parameters', () => {
    const data = mk(new Array(40).fill(50));
    const desc = describeLineUoMidCrossChart(data);
    expect(desc).toContain('UO Mid Cross chart');
...
describe('ChartLineUoMidCross rendering', () => {
  it('renders region + role=img SVG + sr-only desc', () => {
    const data = mk(new Array(40).fill(10));
    const { container, getByRole } = render(
      <ChartLineUoMidCross data={data} />,
    );
    const region = getByRole('region');
    expect(region.getAttribute('aria-label')).toBe('UO Mid Cross chart');
    ...
    expect(desc?.textContent).toContain('UO Mid Cross chart');
  });

  it('renders config badge', () => {
    const data = mk(new Array(40).fill(10));
    const { container } = render(<ChartLineUoMidCross data={data} />);
  ...
  it('respects controlled hiddenSeries', () => {
    ...
    const { container, rerender } = render(
      <ChartLineUoMidCross data={data} hiddenSeries={['uo']} />,
    );
    expect(
    ...
    rerender(<ChartLineUoMidCross data={data} hiddenSeries={[]} />);
    expect(
      container.querySelector(
        '[data-section="chart-line-uo-mid-cross-uo-path"]',
      ),
  ...
  it('empty data -> "No data" empty section', () => {
    const { container } = render(<ChartLineUoMidCross data={[]} />);
    expect(
      container.querySelector(
  ...
  it('animate=false omits motion-safe class', () => {
    const data = mk(new Array(40).fill(10));
    const { container } = render(
      <ChartLineUoMidCross data={data} animate={false} />,
    );
    const svg = container.querySelector('svg');
  ...
  it('showBands=false hides band group', () => {
    const data = mk(new Array(40).fill(10));
    const { container } = render(
      <ChartLineUoMidCross data={data} showBands={false} />,
    );
    expect(
      container.querySelector(
        '[data-section="chart-line-uo-mid-cross-bands"]',
      ),
    ).toBeNull();
  });
  ...
  it('showAxis=false / showGrid=false / showLegend=false', () => {
    ...
    expect(
      container.querySelector(
        '[data-section="chart-line-uo-mid-cross-axes"]',
      ),
    ).toBeNull();
    expect(
      container.querySelector(
        '[data-section="chart-line-uo-mid-cross-grid"]',
      ),
    ).toBeNull();
    expect(
      container.querySelector(
  ...
  it('renders cross markers and overlays after decline then rise pattern', () => {
    ...
    const { container } = render(<ChartLineUoMidCross data={mk(closes)} />);
    const crosses = container.querySelectorAll(
      '[data-section^="chart-line-uo-mid-cross-cross-"]',
    );
  ...
  it('showCrosses=false hides cross markers', () => {
    ...
    const { container } = render(
      <ChartLineUoMidCross data={mk(closes)} showCrosses={false} />,
    );
    expect(
      container.querySelector(
  ...
  it('showOverlayCrosses=false hides overlay arrows', () => {
    ...
    const { container } = render(
      <ChartLineUoMidCross data={mk(closes)} showOverlayCrosses={false} />,
    );
    expect(
      container.querySelector(
  ...
  it('defaults: short=7, mid=14, long=28, threshold=50', () => {
    expect(DEFAULT_CHART_LINE_UO_MID_CROSS_SHORT).toBe(7);
    expect(DEFAULT_CHART_LINE_UO_MID_CROSS_MID).toBe(14);
    expect(DEFAULT_CHART_LINE_UO_MID_CROSS_LONG).toBe(28);
    expect(DEFAULT_CHART_LINE_UO_MID_CROSS_THRESHOLD).toBe(50);
  });

  it('forwardRef returns the wrapping div', () => {
    const data = mk(new Array(40).fill(10));
    const ref = { current: null as HTMLDivElement | null };
    render(<ChartLineUoMidCross data={data} ref={ref} />);
    expect(ref.current).toBeInstanceOf(HTMLDivElement);
    expect(ref.current?.getAttribute('data-section')).toBe(
      'chart-line-uo-mid-cross',
    );
  });

  it('layout is deterministic across calls for default CONST 40 bars', () => {
    const data = mk(new Array(40).fill(10));
    const a = computeLineUoMidCrossLayout({ data });
    const b = computeLineUoMidCrossLayout({ data });
