...
import {
  ChartLineCciCrossSig,
  applyLineCciCrossSigEma,
  applyLineCciCrossSigSma,
  classifyLineCciCrossSigRegime,
  computeLineCciCrossSig,
  computeLineCciCrossSigLayout,
  describeLineCciCrossSigChart,
  detectLineCciCrossSigCrosses,
  getLineCciCrossSigFinitePoints,
  normalizeLineCciCrossSigLength,
  runLineCciCrossSig,
  type ChartLineCciCrossSigPoint,
} from './chart-line-cci-cross-sig';

const constSeries = (n: number, K: number): ChartLineCciCrossSigPoint[] =>
...
describe('getLineCciCrossSigFinitePoints', () => {
  it('returns empty array for null input', () => {
    expect(getLineCciCrossSigFinitePoints(null)).toEqual([]);
  });
  ...
  it('drops NaN and infinite values', () => {
    ...
    expect(getLineCciCrossSigFinitePoints(data)).toEqual([
      { x: 0, close: 10 },
      { x: 2, close: 30 },
    ]);
  });
  ...
  it('drops null entries inside the array', () => {
    const data = [
      { x: 0, close: 1 },
      null as unknown as ChartLineCciCrossSigPoint,
      { x: 1, close: 2 },
    ];
    expect(getLineCciCrossSigFinitePoints(data)).toEqual([
      { x: 0, close: 1 },
...
describe('normalizeLineCciCrossSigLength', () => {
  it('returns fallback when length is below 2', () => {
    expect(normalizeLineCciCrossSigLength(1, 20)).toBe(20);
  });

  it('returns fallback when length is non-number', () => {
    expect(normalizeLineCciCrossSigLength('x', 20)).toBe(20);
  });

  it('returns floored length when acceptable', () => {
    expect(normalizeLineCciCrossSigLength(21.7, 20)).toBe(21);
  });
});

describe('applyLineCciCrossSigSma', () => {
  it('matches the bit-exact CONST K via the precision fix', () => {
    for (const K of [0, 1, 5, 17, 100]) {
      for (const n of [3, 5, 9, 20]) {
        ...
        const out = applyLineCciCrossSigSma(v, n);
...
describe('computeLineCciCrossSig', () => {
  it('handles null series', () => {
    expect(computeLineCciCrossSig(null)).toEqual({ cci: [], signal: [] });
  });

  it('CONST close = K -> CCI = 0, signal = 0 across multiple K', () => {
    for (const K of [0, 1, 5, 17, 100]) {
      const data = constSeries(60, K);
      const { cci, signal } = computeLineCciCrossSig(data, {
        length: 20,
        signalLength: 9,
      });
      const cciWarmup = 20 - 1;
      const sigWarmup = cciWarmup + 9 - 1;
      for (let i = cciWarmup; i < data.length; i += 1) {
        expect(cci[i]).toBe(0);
      }
  ...
  it('falls back to default lengths', () => {
    ...
    const { cci } = computeLineCciCrossSig(data);
    expect(cci[40]).toBe(0);
  });

  it('does not mutate the input series', () => {
    const data = [...constSeries(40, 3)];
    const snapshot = JSON.stringify(data);
    computeLineCciCrossSig(data, { length: 20, signalLength: 9 });
    expect(JSON.stringify(data)).toBe(snapshot);
  });
});

describe('classifyLineCciCrossSigRegime', () => {
  it('returns bullish when cci > signal', () => {
    expect(classifyLineCciCrossSigRegime(2, 1)).toBe('bullish');
  });
  it('returns bearish when cci < signal', () => {
    expect(classifyLineCciCrossSigRegime(0, 1)).toBe('bearish');
  });
  it('returns neutral when cci == signal', () => {
    expect(classifyLineCciCrossSigRegime(1, 1)).toBe('neutral');
  });
  it('returns none when either is null', () => {
    expect(classifyLineCciCrossSigRegime(null, 1)).toBe('none');
    expect(classifyLineCciCrossSigRegime(1, null)).toBe('none');
  });
});

describe('detectLineCciCrossSigCrosses', () => {
  it('flags bullish crosses', () => {
    const series: ChartLineCciCrossSigPoint[] = [
      { x: 0, close: 1 },
      { x: 1, close: 1 },
      { x: 2, close: 1 },
    ];
    const cci = [-1, 1, 1];
    const signal = [0, 0, 0];
    expect(detectLineCciCrossSigCrosses(series, cci, signal)).toEqual([
  ...
  it('skips bars with null cci or signal', () => {
    const series: ChartLineCciCrossSigPoint[] = [
      { x: 0, close: 1 },
      { x: 1, close: 1 },
    ];
    expect(detectLineCciCrossSigCrosses(series, [null, 1], [null, 0])).toEqual([]);
  });
});

describe('runLineCciCrossSig', () => {
  it('returns ok=false for short series', () => {
    const res = runLineCciCrossSig(constSeries(10, 5));
    expect(res.ok).toBe(false);
  });

  it('returns ok=true for long enough series', () => {
    const res = runLineCciCrossSig(constSeries(60, 5));
  ...
  it('CONST K -> regime neutral after warmup, 0 crosses', () => {
    for (const K of [0, 7, 100]) {
      const res = runLineCciCrossSig(constSeries(60, K));
      const sigWarmup = 20 - 1 + 9 - 1;
...
describe('computeLineCciCrossSigLayout', () => {
  it('returns ok=false for empty data', () => {
    const lo = computeLineCciCrossSigLayout({ data: [] });
    expect(lo.ok).toBe(false);
    expect(lo.pricePath).toBe('');
    expect(lo.cciPath).toBe('');
    expect(lo.signalPath).toBe('');
    expect(lo.crossMarkers).toEqual([]);
  });

  it('returns ok=true when data is provided', () => {
    const lo = computeLineCciCrossSigLayout({
  ...
  it('renders price and cci paths', () => {
    const lo = computeLineCciCrossSigLayout({
      data: linearSeries(60, 100, 1),
    });
    expect(lo.pricePath).toMatch(/^M\s/);
    expect(lo.cciPath).toMatch(/^M\s/);
  });
});

describe('describeLineCciCrossSigChart', () => {
  it('returns no-data sentinel when series is empty', () => {
    expect(describeLineCciCrossSigChart([])).toBe('No data');
  });

  it('mentions the bar count', () => {
    const text = describeLineCciCrossSigChart(linearSeries(12, 1, 1));
...
describe('<ChartLineCciCrossSig />', () => {
  it('renders an empty placeholder when data is empty', () => {
    const { container } = render(<ChartLineCciCrossSig data={[]} />);
    expect(
  ...
  it('renders region role when data is present', () => {
    const { container } = render(
      <ChartLineCciCrossSig data={linearSeries(60, 100, 1)} />,
    );
    expect(
      container.querySelector('[data-section="chart-line-cci-cross-sig"]'),
    ).not.toBeNull();
  });
  ...
  it('forwards ref to the wrapper div', () => {
    ...
    render(
      <ChartLineCciCrossSig ref={ref} data={linearSeries(60, 100, 1)} />,
    );
  ...
  it('reports cross count as data attribute', () => {
    const { container } = render(
      <ChartLineCciCrossSig data={constSeries(60, 100)} />,
    );
    const root = container.querySelector(
      '[data-section="chart-line-cci-cross-sig"]',
    ) as HTMLElement | null;
  ...
  it('renders three legend items', () => {
    ...
    const buttons = container.querySelectorAll(
      '[data-section="chart-line-cci-cross-sig-legend"] button',
    );
  ...
  it('hides axes when showAxis=false', () => {
    ...
    expect(
      container.querySelector(
        '[data-section="chart-line-cci-cross-sig-axes"]',
      ),
    ).toBeNull();
  });
  ...
  it('hides grid when showGrid=false', () => {
    ...
    expect(
      container.querySelector(
        '[data-section="chart-line-cci-cross-sig-grid"]',
      ),
    ).toBeNull();
  });
  ...
  it('renders cci and signal paths', () => {
    const { container } = render(
      <ChartLineCciCrossSig data={linearSeries(60, 100, 1)} />,
    );
    expect(
...
describe('CCI Cross Signal integration', () => {
  it('CONST K -> CCI = 0 / signal = 0 / regime neutral / 0 crosses across multiple K', () => {
    const sigWarmup = 20 - 1 + 9 - 1;
    for (const K of [0, 1, 5, 17, 100, 1234]) {
      ...
      for (let i = sigWarmup; i < res.samples.length; i += 1) {
        expect(res.samples[i]?.cci).toBe(0);
